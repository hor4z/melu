...
const marcoOpcion = (elegida: boolean, correcta: boolean, estado: EstadoPaso, revelar: boolean) => {
  if (revelar && correcta) return 'border-success bg-success-subtle kit-correcto'
    ...
    <div className={cn('grid gap-3', ops.length > 3 ? 'sm:grid-cols-2' : 'grid-cols-1')}>
      {ops.map((o, i) => (
        <button key={i} type="button" disabled={estado !== 'editando'} onClick={() => onChange(i)}
          className={cn('kit-reveal flex items-center gap-3 rounded-md border px-4 py-4 text-left text-base transition-colors disabled:cursor-default', ['', 'kit-retraso-1', 'kit-retraso-2', 'kit-retraso-3'][i] ?? '', marcoOpcion(valor === i, b.correcta === i, estado, Boolean(revelar)))}>
      ...
      {(b.opciones ?? []).map((o, i) => {
        ...
        return (
          <button key={i} type="button" disabled={estado !== 'editando'}
            ...
            className={cn('kit-reveal flex items-center gap-3 rounded-md border px-4 py-4 text-left text-base transition-colors disabled:cursor-default', ['', 'kit-retraso-1', 'kit-retraso-2', 'kit-retraso-3'][i] ?? '', marcoOpcion(on, (b.correctas ?? []).includes(i), estado, Boolean(revelar)))}>
      ...
      <div className="flex gap-2">
        {[1, 2, 3, 4, 5].map((n) => (
          <button key={n} type="button" disabled={estado !== 'editando'} onClick={() => onChange(n)}
            className={cn('size-14 rounded-md border text-lg font-semibold transition-transform', valor === n ? 'border-ink bg-solid text-on-solid scale-105' : 'border-line bg-surface hover:border-ink')}>{n}</button>
        ))}
      </div>
