...
--
-- La raya larga se toma dos veces: primero la que viene con espacios alrededor, que es la del
-- texto redactado, y después la suelta. Con un solo reemplazo, una fila cuya única raya fuera
-- "a—b" quedaba igual y la migración se anotaba aplicada: no vuelve a correr nunca.
update activities set document = replace(replace(replace(replace(
  document::text, '«', '\"'), '»', '\"'), ' — ', ': '), '—', ': ')::jsonb
  where document::text ~ '[«»—]';

update assignments set document_snapshot = replace(replace(replace(replace(
